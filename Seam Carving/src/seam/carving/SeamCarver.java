...
public final class SeamCarver{
    ...
    private final double[][] energy;            // energy matrix declaration                     
    
    public SeamCarver(Picture picture){                 // constructor
        this.picture = picture;
        this.energy = new double[height()][width()];    // energy matrix initailization
        
        
        for (int i = 0; i < height(); i++) {            // computing energy
    ...
    public int width(){                         //  return width of image
        return this.picture.width();
    }
     
    public int height(){                        //  return heigth of image
        return this.picture.height();
    }

    public double energy(int y, int x) {                            // energy of pixel at column x and row y
        if (x >= width() || y >= height() || x < 0 || y < 0){       //  if row or coulumn is out of range     
            System.out.println("Boundaries out of Range");
            System.exit(0);
        }
        else if(isBorder(x,y)){                                     //  Border Check
            if(x == 0 && y == 0){                                   // Pixel at (0,0)
            ...
            else if(x == width() - 1 && y == height() - 1){         // Pixel at (heigth()-1, width()-1)
                Color up = new Color(picture.get(x, height() - 1).getRGB());
            ...
            else if(x == width() - 1 && y == 0){                    // Pixel at (0, width()-1)
                Color up = new Color(picture.get(x, height() - 1).getRGB());
                Color down = new Color(picture.get(x, y + 1).getRGB());
                double deltaX = Math.pow(up.getRed() - down.getRed(), 2) + Math.pow(up.getBlue()- down.getBlue(), 2) + Math.pow(up.getGreen()- down.getGreen(), 2);
            ...
            else if(x == 0 && y == height() - 1){                   // Pixel at (heigth()-1, 0)
            ...
            else if(x == width() - 1){                              // Pixel at (any, width()-1)
                Color up = new Color(picture.get(x, y - 1).getRGB());
                Color down = new Color(picture.get(x, y + 1).getRGB());
                double deltaX = Math.pow(up.getRed() - down.getRed(), 2) + Math.pow(up.getBlue()- down.getBlue(), 2) + Math.pow(up.getGreen()- down.getGreen(), 2);
            ...
            else if(y == height() - 1){                             // Pixel at (heigth()-1, any)
                Color up = new Color(picture.get(x, y - 1).getRGB());
                Color down = new Color(picture.get(x, 0).getRGB());
                double deltaX = Math.pow(up.getRed() - down.getRed(), 2) + Math.pow(up.getBlue()- down.getBlue(), 2) + Math.pow(up.getGreen()- down.getGreen(), 2);
    ...
    public boolean isBorder(int x, int y){                                  // Border Checking Function
    ...
    public int[] findHorizontalSeam() {// sequence of indices for horizontal seam
        ...
        for (int i = 0; i < height(); i++) {        //finding min in last column
            if(min > energy[i][width()-1]){         // energy comparison
                min = energy[i][width()-1];         
                index = i;                          // storing index
            }
        } 
        int arr[] = new int[width()];               // seam array declaration
        int arrcount = width() - 1;                 // arrcount is seam array counter
        arr[arrcount] = index;                      // storing minimum index in the array on last index
        arrcount--;                                 // arrcount decrement
       
        
        for(int i = width() - 1; i > 0; i--){       // Traversing the Matrix 
            if(index == 0){                         // Border Condition
                double a = Math.min(energy[index][i-1], energy[index+1][i-1]);      // Minimum of 2 energies are compare
                /*
                    Below is adding in seam array and decrementing arrcount
                */
            ...
            else if(index == height() - 1){         // Border Condition
                double a = Math.min(energy[index][i-1], energy[index-1][i-1]);      // Minimum of 2 energies are compare
                /*
                    Below is adding in seam array and decrementing arrcount
                */
                ...
                double a = Math.min(energy[index+1][i-1], Math.min(energy[index][i-1], energy[index-1][i-1]));  // Minimum of 3 energies are compare
                /*
                    Below is adding in seam array and decrementing arrcount
                */
    ...
    public int[] findVerticalSeam(){ 
        ...
        for (int i = 0; i < width(); i++) {             //finding min in last row
            if(min > energy[height() - 1][i]){          // energy comparison
                min = energy[height() - 1][i];
                index = i;                              // storing index
            }
        } 
        
        int arr[] = new int[height()];                  // seam array declaration 
        int arrcount = height() - 1;                    // arrcount is seam array counter
        arr[height() - 1] = index;                      // storing minimum index in the array on last index
        arrcount--;                                     // arrcount decrement
        
        
        for(int i = height() - 1; i > 0; i--){          // Traversing the Matrix 
            if(index == 0){                             //Border Condition
                double a = Math.min(energy[i-1][index], energy[i-1][index+1]);      // Minimum of 2 energies are compare
                /*
                    Below is adding in seam array and decrementing arrcount
                */
            ...
            else if(index == width() - 1){              //Border Condition
                double a = Math.min(energy[i-1][index], energy[i-1][index-1]);      // Minimum of 2 energies are compare
                /*
                    Below is adding in seam array and decrementing arrcount
                */
                ...
                double a = Math.min(energy[i-1][index-1], Math.min(energy[i-1][index], energy[i-1][index+1]));      // Minimum of 3 energies are compare
                /*
                    Below is adding in seam array and decrementing arrcount
                */
    ...
    public void removeHorizontalSeam(int[] seam){ 
        Picture newpicture = new Picture(width(), height() - 1);        // new picture with height one less than previous
        
        for (int i = 0; i < width(); i++) {      
            // copy pixel of previous image pixel to new image pixel until find seam index
            for (int j = 0; j < seam[i]; j++) {
                newpicture.set(i, j, new Color (picture.get(i, j).getRGB()));
            }
            // copy pixel of previous image pixel after seam index to new image pixel
            for (int j = seam[i]; j < height()-1; j++) {
                newpicture.set(i, j, new Color (picture.get(i, j+1).getRGB()));
            }
	}
        this.picture = newpicture;        // updating class image
        
        // updating energy matrix because old pixels are replace by new ones
    ...
    public void removeVerticalSeam(int[] seam){ 
        Picture newpicture = new Picture(width() - 1, height());        // new picture with width one less than previous
    ...
    public static void main(String[] args){
        /*
            The Java Function and classes use GUI Below is Platform Independent and
            may not work on different OS.
            In case it doesnot work, try to minimize and then maximize the JFrame Application.
